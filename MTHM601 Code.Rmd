...
```{r}
region_data <- spatial_trends_map[complete.cases(spatial_trends_map), ]
...
```{r}
glmer_model <- glmer(roadkill_count ~ mean_temperature + rainfall + (1 | NAME), 
                     family = poisson(link = "log"), data = roadkill_with_climate)
summary(glmer_model)
...
roadkill_points <- st_as_sf(mammal_no_spike, coords = c("long", "lat"), crs = 4326)
...
# Reproject roadkill_points to a projected CRS (e.g., British National Grid: EPSG 27700)
roadkill_points_proj <- st_transform(roadkill_points, crs = 27700)

# Convert to spatstat ppp object
coords_proj <- st_coordinates(roadkill_points_proj)  # Extract projected coordinates
uk_regions_proj <- st_transform(uk_regions, crs = 27700)  # Ensure same CRS for region data
win <- as.owin(st_bbox(uk_regions_proj))  # Create a window object for UK regions
roadkill_ppp <- as.ppp(coords_proj, W = win)
...
neighbors <- knearneigh(coords_proj, k = 8)
nb <- knn2nb(neighbors)

# Compute Getis-Ord Gi* statistic
roadkill_points_proj$roadkill_count <- 1  # Each row is one roadkill
gi_star <- localG(roadkill_points_proj$roadkill_count, nb)

# Add hotspot scores to roadkill points
roadkill_points_proj$hotspot_score <- as.numeric(gi_star)

# Visualize hotspots
ggplot() +
  geom_sf(data = uk_regions_proj, fill = "white") +
  geom_point(data = roadkill_points_proj, aes(x = coords_proj[,1], y = coords_proj[,2], color = hotspot_score)) +
