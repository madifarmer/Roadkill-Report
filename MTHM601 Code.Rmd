...
```{r}
#Combine rain and temp datasets
...
roadkill_climate <- roadkill_climate %>%
  mutate(across(ends_with(".x"), as.numeric)) %>%  # Convert all ".x" columns to numeric
  mutate(across(ends_with(".y"), as.numeric))      # Convert all ".y" columns to numeric
...
```{r}
# Aggregate roadkill counts by season and year
...
# Step 1: Load UK regions shapefile
uk_regions <- st_read("ITL2_JAN_2025_UK_BFE.shp")


# Convert your roadkill data (with latitude and longitude) into a spatial object
roadkill_sf <- mammal_no_spike %>%
  st_as_sf(coords = c("long", "lat"), crs = 4326)  # CRS 4326 is for lat-long

# Ensure the shapefile and roadkill data have the same CRS
uk_regions <- st_transform(uk_regions, crs = 4326)

# Step 4: Perform Spatial Join
# Match roadkill points to regions
roadkill_with_regions <- st_join(roadkill_sf, uk_regions, join = st_intersects)

print(names(uk_regions))

# Aggregate roadkill counts by region
spatial_trends <- roadkill_with_regions %>%
  group_by(ITL225NM) %>%  # Replace 'region' with the column in the shapefile that identifies regions
  summarize(roadkill_count = n(), .groups = "drop")

spatial_trends_map <- uk_regions %>%
  mutate(NAME = as.character(ITL225NM)) %>%  # Ensure `NAME` is comparable
  left_join(as.data.frame(spatial_trends), by = "ITL225NM")

# Aggregate roadkill counts by region name (ITL225NM)
spatial_trends <- roadkill_with_regions %>%
  group_by(ITL225NM) %>%  # Use ITL225NM for names
  summarize(roadkill_count = n(), .groups = "drop")


# Plot spatial trends as a heatmap
tmap_mode("view")
tm_shape(spatial_trends_map) +
  tm_polygons("roadkill_count", 
              title = "Roadkill Count", 
              palette = "YlOrRd", 
              border.alpha = 0.5,
              id = "ITL225NM") +  # Use ITL225NM for tooltips/labels
  tm_layout(main.title = "Spatial Trends in Mammalian Roadkill")
